...
function cleanPick(p) {
  ...
  // ĐỊA CHỈ do định vị trả về lúc thêm quán — thứ tài xế đối chiếu được ngoài đường,
  // và là cách duy nhất để sau này biết chấm đó thật sự nằm ở đâu.
  const addr = str(p.addr, 120);
  return { id, name, cat, lat: +lat.toFixed(5), lng: +lng.toFixed(5), quan: str(p.quan, 40), addr, ts, del: p.del ? 1 : 0, n, win, oto, may, dong, xe };
}
...
function merge(files) {
  ...
  for (const f of files) for (const p of (f.picks || [])) {
    ...
    if (p.ts > cur.ts) { cur.name = p.name; cur.cat = p.cat; cur.lat = p.lat; cur.lng = p.lng; cur.quan = p.quan; cur.del = p.del; cur.ts = p.ts; if (p.xe) cur.xe = p.xe; if (p.addr) cur.addr = p.addr; }
    else { if (p.xe && !cur.xe) cur.xe = p.xe; if (p.addr && !cur.addr) cur.addr = p.addr; }   // máy kia có, máy này chưa → nhận
  }
  ...
  for (const p of all) {
    ...
    if (p.addr && (!hit.addr || p.ts > hit.ts)) hit.addr = p.addr;
